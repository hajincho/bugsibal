...
public class Golem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(batspwan_cooltime >= bat_abletime && dash == false && enemyData.enemy_current_HP > 0 && boom == false)
        {
            ...
            batspwan_cooltime = 0;
            GameObject skeleton1 = Instantiate(skele, new Vector2(transform.position.x -3, transform.position.y), Quaternion.identity);
            GameObject skeleton2 = Instantiate(skele, new Vector2(transform.position.x, transform.position.y + 3), Quaternion.identity);
            GameObject skeleton3 = Instantiate(skele, new Vector2(transform.position.x + 3, transform.position.y), Quaternion.identity);
        ...
        if (enemyData.enemy_current_HP <= 0)
        {
            ...
            if (!isdead)
            {
                Item itemboss = itemdrop.Uniquedrop();
                itemdrop.ItemPrefInst(transform.position, itemboss.value, itemboss);
                isdead = true;
            }
